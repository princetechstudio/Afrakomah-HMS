...
export async function loadRelational(): Promise<DB> {
  const [staff, patients, wards, beds, appointments, vitals, consultations, patientDocuments, patientComments, labTests, labOrders, labResults, medicines, prescriptions, prescriptionItems, admissions, maternity, nursing, invoices, invoiceItems, payments, notifications, audit] = await Promise.all([
    rows("staff"), rows("patients"), rows("wards"), rows("beds"), rows("appointments"), rows("vitals"), rows("consultations"), rows("patient_documents"), rows("patient_comments"), rows("lab_tests"), rows("lab_orders"), rows("lab_results"), rows("medicines"), rows("prescriptions"), rows("prescription_items"), rows("admissions"), rows("maternity_records"), rows("nursing_notes"), rows("invoices"), rows("invoice_items"), rows("payments"), rows("notifications"), rows("audit_logs"),
  ]);
