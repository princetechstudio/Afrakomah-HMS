...
create table public.treatment_entries (id uuid primary key default gen_random_uuid(), external_id text not null unique, patient_id uuid not null references public.patients(id) on delete cascade, medicine text not null, dose text not null default '', route text not null default '', frequency text not null default '', times text[] not null default '{}', treatment_date date not null, instructions text not null default '', administrations jsonb not null default '{}', created_at timestamptz not null default now(), created_by uuid not null references public.staff(id));
